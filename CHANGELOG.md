...
- `content_type.is_supported_request` and `is_supported_response` now consult the capability registry instead of pattern-matching on individual `ContentType` variants. `UnsupportedContentType(_)` still short-circuits to False. A mirrored drift test covers the request side so the registry and helpers stay in lockstep on both modes (#175, subset of #102).
