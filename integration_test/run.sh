...
# blank line, trailing space, etc.) would otherwise slip through.
#
# Several tests overwrite `handlers.gleam` with a hand-written stub
# *after* generation — that file is intentionally excluded so the gate
# only judges what `oaspec generate` itself emitted. Tests that don't
# overwrite the stub are also covered, since their `handlers.gleam`
# is the auto-formatted generator output.
#
# Usage: invoke from the test's project directory after generation,
# passing the directory `oaspec generate` emitted into (typically
# `src/api`).
verify_generated_format() {
  ...
  # Smoke 1: every generated `.gleam` file is non-empty. A
  # zero-length file slips past `gleam build --warnings-as-errors`
  # with only an "empty module" warning from the compiler, so we
  ...
  # Smoke 2: every generated `.gleam` file carries the
  # `// Code generated by oaspec` provenance header. Missing it
  # implies the file did not flow through `oaspec generate` —
  # either the test wrote a stub there by mistake, or the codegen
  # forgot to prepend `se.file_header`.
  while IFS= read -r f; do
    if ! head -n1 "$f" | grep -q "// Code generated by oaspec"; then
      fail "Generated file $f is missing the codegen provenance header (codegen regression)."
    fi
  done <<< "$files"

  # Format gate: `oaspec generate` runs `gleam format` during
  # emission, so the produced sources must already pass a no-op
  # format check; a regression in the formatter step (stray blank
  # line, trailing space, etc.) is caught here.
  # shellcheck disable=SC2086 -- intentional word-splitting on find output
  gleam format --check $files || fail "Generated code under $target_dir is not gleam-formatted (regression)."
}
...
verify_generated_format src/api
