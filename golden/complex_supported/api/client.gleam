...
/// Complex search
pub fn post_search(config: ClientConfig, body: Option(types.PostSearchRequest)) -> Result(response_types.PostSearchResponse, ClientError) {
  let path = "/search"
...
/// Receive webhook
pub fn post_webhook(config: ClientConfig, body: Option(types.WebhookEvent)) -> Result(response_types.PostWebhookResponse, ClientError) {
  let path = "/webhook"
  let assert Ok(req) = request.to(config.base_url <> path)
  let req = request.set_method(req, http.Post)
  let req = case body {
